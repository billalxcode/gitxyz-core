...
func TestCreatorCanAccessPrivateRepo(t *testing.T) {
	...
	owner, _ := userRepo.FindByUsername("owner1")
	repoRepo := repository.NewRepoRepository(db)
	_ = repoRepo.Create(&models.Repository{Name: "priv", IsPrivate: true, UserID: owner.ID.String()})

	perm := NewPermission(db)
	ctx, _ := gin.CreateTestContext(nil)
	ctx.Set("username", "owner1")
	ctx.Set("user_id", owner.ID.String())
	ctx.Set("role", "user")
	ctx.Set("token_type", "jwt")
	ctx.Set("token_scopes", "")
